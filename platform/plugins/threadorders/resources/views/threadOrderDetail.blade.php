...
@section('content')
    <div class="p-3 bg-white" >
        ...
        <div id="main">
            ...
            <div class="row">
                <div class="col-lg-3">
                    <p class="m-0 heading color-white">Thread Status</p>
                    <p class="text-center">{{$orderDetail->thread_status}}</p>
                </div>
                <div class="col-lg-3">
                    <p class="m-0 heading color-white">Vendor</p>
                    <p class="text-center">{{$orderDetail->vendor->getFullName()}}</p>
                </div>
                @if($orderDetail->thread_status == \Botble\Thread\Models\Thread::PRIVATE && $orderDetail->pvt_customer_id)
                    <div class="col-lg-3">
                        <p class="m-0 heading color-white">Pvt. Customer</p>
                        <p class="text-center">{{$orderDetail->pvt_customer->name}}</p>
                    </div>
                @endif
                <div class="col-lg-3">
                    <p class="m-0 heading color-white">Order No.</p>
                    <p class="text-center">{{$orderDetail->order_no}}</p>
                </div>
                <div class="col-lg-3">
                    <p class="m-0 heading color-white"> Regular Pack Category</p>
                    <p class="text-center">{{@$orderDetail->threadOrderVariations(\Botble\Thread\Models\Thread::REGULAR)[0]->cat_name}}</p>
                </div>
                @if(isset($orderDetail->threadOrderVariations(\Botble\Thread\Models\Thread::PLUS)[0]->cat_name))
                    <div class="col-lg-3">
                        <p class="m-0 heading color-white"> Plus Pack Category</p>
                        <p class="text-center">{{@$orderDetail->threadOrderVariations(\Botble\Thread\Models\Thread::PLUS)[0]->cat_name}}</p>
                    </div>
                @endif
                <div class="col-lg-3">
                    <p class="m-0 heading color-white">Description</p>
                    <p class="text-center">{{$orderDetail->name}}</p>
                </div>
                <div class="col-lg-3">
                    <p class="m-0 heading color-white"> PP Sample</p>
                    <p class="text-center">{{$orderDetail->pp_sample}}</p>
                </div>
                <div class="col-lg-3">
                    <p class="m-0 heading color-white">PP Sample Date</p>
                    @if($orderDetail->pp_sample == \Botble\Thread\Models\Thread::YES)
                        <p class="text-center">{{date('d F, Y', strtotime($orderDetail->pp_sample_date))}}</p>
                    @else
                ...
                @if($orderDetail->material)
                    <div class="col-lg-3">
                        <p class="m-0 heading color-white">Fabric</p>
                        <p class="text-center">{{$orderDetail->material}}</p>
                    </div>
                @endif
                <div class="col-lg-3">
                    <p class="m-0 heading color-white">Select Shipping Method</p>
                    <p class="text-center">{{$orderDetail->shipping_method}}</p>
                </div>
                @if($orderDetail->vendor_product)
                    <div class="col-lg-3">
                        <p class="m-0 heading color-white">Vendor Product</p>
                        <p class="text-center">{{$orderDetail->vendor_product->name}}</p>
                    </div>
                @endif
            </div>
                ...
                <div class="row">
                    ...
                    <li>
                        ...
                        <div class="col-lg-3">
                            ...
                            <p class="text-center">{{$variation->sku}}</p>
                        </div>
                        <div class="col-lg-2">
                            <p class="m-0 heading color-white">Type</p>
                            <p class="text-center">{{$variation->category_type}}</p>
                        </div>
                        <div class="col-lg-2">
                            <p class="m-0 heading color-white">{{$orderDetail->is_pieces ? 'Pieces Qty' : 'Pack Qty'}}</p>
                            <p class="text-center">{{$variation->quantity}}</p>
                        </div>
                        <div class="col-lg-1">
                            <p class="m-0 heading color-white">Cost</p>
                            <p class="text-center">{{$variation->cost}}</p>
                        </div>
                        <div class="col-lg-2">
                            <p class="m-0 heading color-white">Per Piece Qty</p>
                            <p class="text-center">{{$variation->per_piece_qty}} {{$variation->unit_name}}</p>
                        </div>
                        <div class="col-lg-2">
                            <p class="m-0 heading color-white">UPC</p>
                            <p class="text-center">{{$variation->upc}}</p>
                        </div>
                        <div class="col-lg-12">
                            <p class="m-0 heading color-white">Barcode</p>
                    ...
                    <div class="row">
                        @if($product)
                            @foreach($product->variations as $prod_variation)
                                ...
                                <div class="col-lg-2">
                                    <p class="m-0 heading color-white">SKU</p>
                                    <p class="text-center">{{$prod_variation->product->sku}}</p>
                                </div>
                                <div class="col-lg-2">
                                    <p class="m-0 heading color-white">UPC</p>
                                    <p class="text-center">{{$prod_variation->product->upc}}</p>
                                </div>
                                <div class="col-lg-6">
                                <div class="d-flex">
