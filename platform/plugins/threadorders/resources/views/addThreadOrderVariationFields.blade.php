...
@foreach($options['data']->thread_variations as $variation)
    ...
    <div class="col-md-4">
        <div class="form-group">
            <label for="regular_qty" class="control-label">{{$options['data']->is_pieces ? 'Reg. Pieces Qty' : 'Reg. Pack Qty'}}</label>
            <input class="form-control" name="regular_qty[{{$variation->id}}]" type="number" value="{{$variation->regular_qty}}">
        </div>
    </div>
    <div class="col-md-4">
        <div class="form-group">
            <label for="plus_qty" class="control-label">{{$options['data']->is_pieces ? 'Plus Pieces Qty' : 'Plus Pack Qty'}}</label>
