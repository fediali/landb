<div id="denim-fields">
    ...
    <div class="form-group" id="inseam-div">
        ...
        <input class="form-control" placeholder="Inseam" data-counter="120" name="inseam" type="text" id="inseam"
               value="{{$options['data']['model']->inseam}}">
    </div>
        ...
        <div class="form-group">
            <select class="select-search-full" id="fit_id" name="fit_id">
                ...
                @foreach($options['data']['fits'] as $key => $fit)
                    <option
                        value="{{$key}}" {{$options['data']['model']->fit_id == $key ? 'selected' : ''}}>{{$fit}}</option>
                @endforeach
            </select>
        </div>
    </div>
        ...
        <div class="form-group">
            <select class="select-search-full" id="rise_id" name="rise_id">
                ...
                @foreach($options['data']['rises'] as $key => $rise)
                    <option
                        value="{{$key}}" {{$options['data']['model']->rise_id == $key ? 'selected' : ''}}>{{$rise}}</option>
                @endforeach
            </select>
        </div>
    </div>
        ...
        <div class="form-group">
            <select class="select-search-full" id="fabric_id" name="fabric_id">
                ...
                @foreach($options['data']['fabrics'] as $key => $fabric)
                    <option
                        value="{{$key}}" {{$options['data']['model']->fabric_id == $key ? 'selected' : ''}}>{{$fabric}}</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <div class="form-group" id="fabric_print_direction-div">
        ...
        <input class="form-control" placeholder="Fabric Print Direction" data-counter="120"
               name="fabric_print_direction" type="text" id="fabric_print_direction"
               value="{{$options['data']['model']->fabric_print_direction}}">
    </div>

    <div class="form-group">
        <label for="reg_pack_qty" class="control-label">Reg Pack Qty</label>
        <input class="form-control" placeholder="Reg Pack Qty" name="reg_pack_qty" type="number" id="reg_pack_qty"
               value="{{$options['data']['model']->reg_pack_qty}}">
    </div>

    <div class="form-group">
        <label for="plus_pack_qty" class="control-label">Plus Pack Qty</label>
        <input class="form-control" placeholder="Plus Pack Qty" name="plus_pack_qty" type="number" id="plus_pack_qty"
               value="{{$options['data']['model']->plus_pack_qty}}">
    </div>
