...
class ReturnReportsTable extends TableAbstract
{
     ...
     * @var string
     */
    protected $customFilterTemplate = 'plugins/ecommerce::orders.returnOrderFilter';
    ...
    public function query()
    {
        ...
        $select = [
            ...
            'ec_order_refund_product.qty',
            'ec_order_refund_product.price',
        ];

        $query = $model->select($select)
            ->join('ec_order_refund_product', 'ec_order_refund_product.product_id', 'ec_products.id')
        ...
        if (!empty($search_items)) {
            if (isset($search_items['from_date'])) {
                $query->whereDate('ec_products.created_at', '>=', Carbon::createFromFormat('m-d-Y', $search_items['from_date'])->format('Y-m-d'));
            }
            if (isset($search_items['to_date'])) {
                $query->whereDate('ec_products.created_at', '<=', Carbon::createFromFormat('m-d-Y', $search_items['to_date'])->format('Y-m-d'));
            }
        }
    ...
    public function columns()
    {
        $arr = [
            ...
            'qty'      => [
                'name'  => 'ec_order_refund_product.qty',
                'title' => 'Returned Qty',
                'class' => 'text-left',
            ],
            'price'      => [
                'name'  => 'ec_order_refund_product.price',
