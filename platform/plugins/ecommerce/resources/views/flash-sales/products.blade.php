<div class="form-group">
    <input type="hidden" name="products" value="@if ($flashSale->id) {{ implode(',', $flashSale->products()->allRelatedIds()->toArray()) }} @endif" />
    <div class="box-search-advance product">
        <div>
            <input type="text" class="next-input textbox-advancesearch" placeholder="{{ trans('plugins/ecommerce::products.search_products') }}" data-target="{{ route('products.get-list-product-for-search', 0) }}">
        </div>
                ...
                @foreach($products as $index => $product)
                    <tr data-product-id="{{ $product->id }}">
                        ...
                        <td class="pl5 p-r5 min-width-200-px" style="padding-top: 15px;">
                            <a class="hover-underline pre-line" href="{{ route('products.edit', $product->id) }}" target="_blank">{{ $product->name }}</a>
                        </td>
                        <td class="pl5 p-r5 text-right width-20-px min-width-20-px" style="padding-top: 15px;">
                    ...
                    <tr data-product-id="{{ $product->id }}">
                        <td colspan="3" style="border-top: none; @if (!$loop->last) border-bottom: 1px solid #ececec; @endif">
                            <div class="row">
                                <div class="col-6">
                                    <div class="form-group">
        ...
        <td colspan="3" style="border-top: none; border-bottom: 1px solid #ececec;">
            <div class="row">
                <div class="col-6">
                    <div class="form-group">
                        <label class="control-label required">{{ trans('plugins/ecommerce::products.price') }}</label>
                        <input type="text" class="form-control input-mask-number" name="products_extra[__index__][price]">
                    </div>
                </div>
                <div class="col-6">
                    <div class="form-group">
                        <label class="control-label required">{{ trans('plugins/ecommerce::products.quantity') }}</label>
                        <input type="text" class="form-control" name="products_extra[__index__][quantity]">
                    </div>
                </div>
            </div>
        </td>
