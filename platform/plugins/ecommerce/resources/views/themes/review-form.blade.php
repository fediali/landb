    ...
    <div class="col-md-6 col-12">
        ...
        {!! Form::open(['route' => 'public.reviews.create', 'method' => 'post']) !!}
            ...
            <div class="form-group {{ $errors->has('star') ? ' has-error' : '' }}">
                ...
                <input type="hidden" name="star" id="rating" @if (!auth('customer')->check() || ($is_reviewed = check_if_reviewed_product($productId))) disabled value="{{ get_customer_reviewed_value($productId) }}" @endif/>
            </div>

            @if (!isset($is_reviewed) || !$is_reviewed)
