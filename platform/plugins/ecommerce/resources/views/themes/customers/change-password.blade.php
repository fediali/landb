...
@section('content')
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            ...
            <div class="mt-3 @if ($errors->has('old_password')) has-error @endif">
                <span class="input-group-prepend">{{ __('Old password:') }}</span>
                <input type="password" class="form-control" name="old_password" id="old_password"
                       placeholder="{{ __('Current Password') }}">
                {!! Form::error('old_password', $errors) !!}
            </div>
            <div class="mt-3 @if ($errors->has('password')) has-error @endif">
                <span class="input-group-prepend">{{ __('New password:') }}</span>
                <input type="password" class="form-control" name="password" id="password"
                       placeholder="{{ __('New Password') }}">
                {!! Form::error('password', $errors) !!}
            </div>
            <div class="mt-3 @if ($errors->has('password_confirmation')) has-error @endif">
                <span class="input-group-prepend">{{ __('Password confirmation:') }}</span>
                <input type="password" class="form-control" name="password_confirmation" id="password_confirmation"
                       placeholder="{{ __('Password Confirmation') }}">
                {!! Form::error('password_confirmation', $errors) !!}
            </div>
            <div class="mt-3 form-group text-right p-0">
                <button type="submit" class="btn btn-primary btn-sm btn-add-address">{{ __('Change password') }}</button>
            </div>
            {!! Form::close() !!}
        </div>
    </div>
