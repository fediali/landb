        ...
        @foreach($prodSkus as $sku)
            ...
            <div class="tab-pane fade show {{$loop->first ? 'active' : ''}}" id="{{$sku}}" role="tabpanel"
                ...
                @php
                        ...
                        ->join('ec_orders', 'ec_orders.id', 'ec_order_product.order_id')->where(['ec_orders.order_type'=> \Botble\Ecommerce\Models\Order::NORMAL, 'status'=>\Botble\Base\Enums\BaseStatusEnum::CANCELLED])->where('ec_products.sku', $sku)->sum('ec_order_product.qty');
                @endphp
