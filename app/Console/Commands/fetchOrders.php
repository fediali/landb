...
class fetchOrders extends Command
{
    ...
    public function handle()
    {
        DB::connection('mysql2')
            ->table('hw_orders')
            ->where('hw_orders.fetch_status', 0)
            ->where(function ($q) {
                $q->where('status', 'AJ')->orWhere('status', 'AY');
            })
            ->orderBy('hw_orders.order_id', 'ASC')
            ->chunk(500,
                function ($orders) {
                    foreach ($orders as $order) {
                        $checkOrder = Order::where('id', $order->order_id)->first();
                        if ($checkOrder && $order->online_order) {
                            echo $order->order_id;
                            $pre = 0;
                            if ($order->status == 'C') {
                            ...
                            $onlineorder = Order::create($orderData);
                            $neworder = Order::where('id', $onlineorder->id)->first();
                            ...
                            foreach ($orderProducts as $orderProduct) {
                                ...
                                if ($productObj && $productObj->prod_pieces) {//3
                                    ...
                                    if ($qty > 0) {
                                        ...
                                        $meta_condition = ['order_id' => $onlineorder->id];
                                        $paymentData = [
                                        ...
                                        Order::where('id', $onlineorder->id)->update(['payment_id' => $payment->id]);
...
//                                'updated_at' => date('Y-m-d H:i:s', $order->last_status_change_date),
//                            ];
//                            DB::table('ec_orders')->where('id', $order->order_id)->update($orderData);
//
//                            $neworder = Order::where('id', $order->order_id)->first();
//
//                            $orderProducts = DB::connection('mysql2')->table('hw_order_details')->where('order_id', $order->order_id)->get();
